...
|Daum Postcode API|1. 우편번호, 도로명, 지번 주소 데이터를 얻어올 수 있으며, JSON 형식의 데이터를 리턴한다.|
|Naver Map API|1. 네이버 지도에 위도, 경도를 기반으로 한 위치를 표시할 수 있다.<br>2. 위치 변경, 주소를 기반으로 한 위경도 추출, 위경도를 기반으로 한 주소 추출 작업을 할 수 있으며, JSON 형식의 데이터를 리턴한다.|
|Backend-공용|1. multer를 이용한 이미지 사진 등록 기능 구현|
|Backend|1. Users - 회원가입, 로그인, 회원 정보 수정, 회원 탈퇴 기능 제공<br>2. Users - 사용자의 비밀번호는 암호화하여 저장(Bcrypt 이용)<br>3. Users - 로그인 시 토큰을 생성(JWT 활용)<br>4. Users - 생성된 토큰은 각종 기능의 사용자 인증에 활용(Mate 글쓰기, 회원 정보 조회 등)<br>5. Animals - 회원의 반려동물 정보 등록, 조회, 수정, 삭제 기능 제공<br>6. Animals - 등록한 반려동물 정보는 Mate 등에서 활용<br>7. MateBoard - 글 쓰기, 읽기, 수정, 삭제 기능 제공<br>8. MateBoard - 게시글 지역별 필터(시 단위, 구 단위), 날짜별 정렬 기능 제공<br>9. MateBoard - 게시글에 대하여 offset, limit을 이용한 페이지네이션 데이터 제공|
